...
Due to the nature of observables, the `selectAll()` operator will map over the entities twice, once for each subscription. We can use the `shareReplay` operator to optimize it:
...
With this change, the `selectAll` operator will now share the result with every subscriber.
