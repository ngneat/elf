...
todosStore.actions$.pipe(ofType(EntityActions.Set)).subscribe((newIds) => {});
todosStore.actions$.pipe(ofType(EntityActions.Add)).subscribe((addedIds) => {});
todosStore.actions$
  .pipe(ofType(EntityActions.Update))
  .subscribe((updatedIds) => {});
todosStore.actions$
  .pipe(ofType(EntityActions.Remote))
...
todosStore.actions$.subscribe((action) => {});
```
