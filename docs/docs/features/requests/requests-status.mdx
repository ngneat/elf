...
import { withRequestsStatus, createRequestsStatusOperator } from '@ngneat/elf-requests';
...
const todosStore = new Store({ name: 'todos', state, config });
// highlight-next-line
export const trackTodosRequestsStatus = createRequestsStatusOperator(todosStore);
```
...
store.update(updateRequestsStatus({
  keyOne: {
    value: 'success'
  }
}));
