...
The initial value of any `key` is `idle`. `setRequestStatus` changes the value to `pending` or to `error` when it fails. Upon successful completion, the `success` status must be manually set as follows:
