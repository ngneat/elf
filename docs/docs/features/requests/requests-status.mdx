...
In your server call, you can use the `trackRequestStatus` operator and pass a unique `key` to identify the request:
...
export function fetchTodos() {
  return http.get(todosUrl).pipe(
    ...
    // highlight-next-line
    trackRequestStatus(todosStore, 'todos')
  );
}
```
...
```ts
trackRequestStatus(todosStore, 'todos', { handleSuccess: true })
```

You can use the `initializeAsIdle` function to initialize a request as `idle`:

```ts
import { createState, Store } from '@ngneat/elf';
import { withEntities } from '@ngneat/elf-entities';
import { withRequestsStatus, initializeAsIdle } from '@ngneat/elf-requests';
...
const { state, config } = createState(
  ...
  withRequestsStatus(
    // highlight-next-line
    initializeAsIdle(['addTodo', 'updateTodo'])
  )
);
