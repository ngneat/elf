...
export function stateFactory<T extends Record<any, any>, K extends keyof T = T extends Record<infer Key, any> ? Key : never>(key: K, defaultValue: T[K]) {
  ...
  return {
    ...
    P extends `set${Capitalize<string & K>}` ?
      <S extends T>(value: S[K] | ((state: S) => S)) => Reducer<S> : P extends `select${Capitalize<string & K>}` ? <S extends T>() => OperatorFunction<S, S[K]> :
