...
describe('devtools', () => {
  ...
  it('should send actions', () => {
    ...
    expect(instanceMock.send).toHaveBeenCalledWith({ type: '@Init Todos' }, {
      'todos': {
        '$entities': {},
        '$ids': []
      }
    ...
    expect(instanceMock.send).toHaveBeenCalledWith({ type: 'Update Todos' }, {
      'todos': {
        '$entities': {
          1: {
            'completed': false,
    ...
    expect(instanceMock.send).toHaveBeenCalledWith({ type: 'Remove Todos' }, {});

    createEntitiesStore('foo');

    expect(instanceMock.send).toHaveBeenCalledWith({ type: '@Init Foo' }, {
      'foo': {
        '$entities': {},
        '$ids': []
      }
    ...
    expect(instanceMock.send).toHaveBeenCalledWith({ type: '@Init Bar' }, {
      'foo': {
        '$entities': {},
        '$ids': []
      },
    ...
    expect(instanceMock.send).toHaveBeenCalledWith({ type: 'Update Bar' }, {
      'foo': {
        '$entities': {},
        '$ids': []
      },
